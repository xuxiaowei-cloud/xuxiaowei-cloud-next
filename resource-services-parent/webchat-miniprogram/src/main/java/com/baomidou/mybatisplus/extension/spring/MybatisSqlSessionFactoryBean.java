...
public class MybatisSqlSessionFactoryBean
	...
	/**
	 * Sets the DatabaseIdProvider. As of version 1.2.2 this variable is not initialized
	 * by default.
	...
	/**
	 * Super class which domain objects have to extend to have a type alias created. No
	 * effect if there is no package to scan configured.
	...
	/**
	 * Set type handlers. They must be annotated with {@code MappedTypes} and optionally
	 * with {@code MappedJdbcTypes}
	 ...
	 * statements are fully loaded and there is no one still pending to resolve includes.
	 * Defaults to false.
	...
	/**
	 * Set the location of the MyBatis {@code SqlSessionFactory} config file. A typical
	...
	/**
	 * Set a customized MyBatis configuration. TODO 这里的入参使用 MybatisConfiguration 而不是
	 * Configuration
	 ...
	 * <p>
	 * This is an alternative to specifying "&lt;sqlmapper&gt;" entries in an MyBatis
	 * config file. This property being based on Spring's resource abstraction also allows
	 * for specifying resource patterns here: e.g. "classpath*:sqlmap/*-mapper.xml".
	 ...
	 * alternative to a {@code &lt;properties&gt;} tag in the configuration xml file. This
	 * will be used to resolve placeholders in the config file.
	 ...
	 * The {@code DataSource} should match the one used by the {@code SqlSessionFactory}:
	 ...
	 * <p>
	 * The {@code DataSource} specified here should be the target {@code DataSource} to
	 * manage transactions for, not a {@code TransactionAwareDataSourceProxy}. Only data
	 * access code may work with {@code TransactionAwareDataSourceProxy}, while the
	 * transaction manager needs to work on the underlying target {@code DataSource}. If
	 * there's nevertheless a {@code TransactionAwareDataSourceProxy} passed in, it will
	 * be unwrapped to extract its target {@code DataSource}.
	 ...
	 * <p>
	 * The default {@code SpringManagedTransactionFactory} should be appropriate for all
	 * cases: be it Spring transaction management, EJB CMT or plain JTA. If there is no
	 * active transaction, SqlSession operations will execute SQL statements
	 * non-transactionally.
	 *
	 * <b>It is strongly recommended to use the default {@code TransactionFactory}.</b> If
	 * not used, any attempt at getting an SqlSession through Spring's MyBatis framework
	 * will throw an exception if a transaction is active.
	 ...
	 * <p>
	 * The default implementation uses the standard MyBatis {@code XMLConfigBuilder} API
	 * to build a {@code SqlSessionFactory} instance based on an Reader. Since 1.3.0, it
	 * can be specified a {@link Configuration} instance directly(without config file).
	 * </p>
	...
	protected SqlSessionFactory buildSqlSessionFactory() throws Exception {
		...
		this.globalConfig.setDbConfig(
				Optional.ofNullable(this.globalConfig.getDbConfig()).orElseGet(GlobalConfig.DbConfig::new));
		...
		if (this.databaseIdProvider != null) {// fix #64 set databaseId before parse
												// mapper xmls
			try {
